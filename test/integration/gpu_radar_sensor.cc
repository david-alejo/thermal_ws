...
sdf::ElementPtr GpuRadarToSdf(const std::string &name,
    ...
    const bool visualize)
{
  ...
  stream
    ...
    << " <model name='m1'>"
    << "  <link name='link1'>"
    << "    <sensor name='" << name << "' type='gpu_radar'>"
    << "      <pose>" << pose << "</pose>"
    << "      <topic>" << topic << "</topic>"
...
void OnNewRadarFrame(const float * /*_scan*/, unsigned int /*_width*/,
...
class GpuRadarSensorTest: public testing::Test,
                  public testing::WithParamInterface<const char *>
{
  // Test and verify gpu rays properties setters and getters
  public: void CreateGpuRadar(const std::string &_renderEngine);
  ...
  public: void VerticalRadar(const std::string &_renderEngine);
...
/////////////////////////////////////////////////
/// \brief Test Creation of a GPU Radar sensor
void GpuRadarSensorTest::CreateGpuRadar(const std::string &_renderEngine)
{
  // Create SDF describing a camera sensor
  const std::string name = "TestGpuRadar";
  const std::string parent = "parent_link";
  const std::string topic = "/ignition/sensors/test/radar";
  ...
  sdf::ElementPtr radarSdf = GpuRadarToSdf(name, testPose, updateRate, topic,
  ...
  // Create a GpuRadarSensor
  ignition::sensors::GpuRadarSensor *sensor =
      mgr.CreateSensor<ignition::sensors::GpuRadarSensor>(radarSdf);
  sensor->SetParent(parent);
  // Make sure the above dynamic cast worked.
  ASSERT_NE(nullptr, sensor);
  sensor->SetScene(scene);

  // Set a callback on the radar sensor to get a scan
  ignition::common::ConnectionPtr c =
    sensor->ConnectNewRadarFrame(
        std::bind(&::OnNewRadarFrame,
...
void GpuRadarSensorTest::DetectBox(const std::string &_renderEngine)
{
  // Create SDF describing a camera sensor
  const std::string name = "TestGpuRadar";
  const std::string parent = "parent_link";
...
void GpuRadarSensorTest::TestThreeBoxes(const std::string &_renderEngine)
{
  // Test GPU radar sensor with 3 boxes in the world.
  ...
  // Create SDF describing a gpu radar sensors sensor
  const std::string name1 = "TestGpuRadar1";
  const std::string name2 = "TestGpuRadar2";
  const std::string topic1 = "/ignition/sensors/test/radar1";
  const std::string topic2 = "/ignition/sensors/test/radar2";
  const double updateRate = 30;
  const int horzSamples = 320;
  const double horzResolution = 1;
  const double horzMinAngle = -IGN_PI/2.0;
  ...
  ignition::math::Pose3d testPose1(ignition::math::Vector3d(0, 0, 0.1),
      ignition::math::Quaterniond::Identity);
  sdf::ElementPtr radarSdf1 = GpuRadarToSdf(name1, testPose1, updateRate,
  ...
  sdf::ElementPtr radarSdf2 = GpuRadarToSdf(name2, testPose2, updateRate,
  ...
  // Create a GpuRadarSensors
  ignition::sensors::GpuRadarSensor *sensor1 =
      mgr.CreateSensor<ignition::sensors::GpuRadarSensor>(radarSdf1);

  // Create second GpuRadarSensor
  ignition::sensors::GpuRadarSensor *sensor2 =
      mgr.CreateSensor<ignition::sensors::GpuRadarSensor>(radarSdf2);
  ...
  // ign-rendering uses lower resolution textures for radars with low sample
...
/////////////////////////////////////////////////
/// \brief Test detect one box
void GpuRadarSensorTest::VerticalRadar(const std::string &_renderEngine)
{
  // Create SDF describing a camera sensor
  const std::string name = "TestGpuRadar";
  const std::string topic = "/ignition/sensors/test/radar";
...
void GpuRadarSensorTest::ManualUpdate(const std::string &_renderEngine)
{
  // Create SDF describing a gpu radar sensors sensor
  const std::string name1 = "TestGpuRadar1";
  const std::string name2 = "TestGpuRadar2";
  ...
  // Create testing box
  // box in the center of radar1 and right of radar2
  ignition::math::Pose3d box01Pose(ignition::math::Vector3d(1, 0, 0.5),
      ignition::math::Quaterniond::Identity);
  ignition::rendering::VisualPtr visualBox1 = scene->CreateVisual("TestBox1");
  visualBox1->AddGeometry(scene->CreateBox());
...
/////////////////////////////////////////////////
void GpuRadarSensorTest::Topic(const std::string &_renderEngine)
{
  const std::string name = "TestGpuRadar";
  const double updateRate = 30;
  const unsigned int horzSamples = 640;
  ...
  // Default topic
  {
    ...
    auto radarSdf = GpuRadarToSdf(name, testPose, updateRate, topic,
      horzSamples, horzResolution, horzMinAngle, horzMaxAngle,
      vertSamples, vertResolution, vertMinAngle, vertMaxAngle,
      rangeResolution, rangeMin, rangeMax, alwaysOn, visualize);

    auto radar = mgr.CreateSensor<ignition::sensors::GpuRadarSensor>(radarSdf);
    ASSERT_NE(nullptr, radar);
    EXPECT_NE(ignition::sensors::NO_SENSOR, radar->Id());
    EXPECT_EQ("/radar/points", radar->Topic());
  }
  ...
  // Convert to valid topic
  {
    ...
    EXPECT_EQ("/topic_with_spaces/characters/points", radar->Topic());
  }
  ...
  // Invalid topic
  {
    ...
    auto sensor = mgr.CreateSensor<ignition::sensors::GpuRadarSensor>(radarSdf);
...
/////////////////////////////////////////////////
#ifdef __APPLE__
TEST_P(GpuRadarSensorTest, DISABLED_CreateGpuRadar)
#else
TEST_P(GpuRadarSensorTest, CreateGpuRadar)
#endif
{
  CreateGpuRadar(GetParam());
}

/////////////////////////////////////////////////
#ifdef __APPLE__
TEST_P(GpuRadarSensorTest, DISABLED_DetectBox)
#else
TEST_P(GpuRadarSensorTest, DetectBox)
#endif
{
  DetectBox(GetParam());
}
...
/////////////////////////////////////////////////
#ifdef __APPLE__
TEST_P(GpuRadarSensorTest, DISABLED_TestThreeBoxes)
#else
TEST_P(GpuRadarSensorTest, TestThreeBoxes)
#endif
{
...
/////////////////////////////////////////////////
#ifdef __APPLE__
TEST_P(GpuRadarSensorTest, DISABLED_VerticalRadar)
#else
TEST_P(GpuRadarSensorTest, VerticalRadar)
#endif
{
  VerticalRadar(GetParam());
}

/////////////////////////////////////////////////
#ifdef __APPLE__
TEST_P(GpuRadarSensorTest, DISABLED_ManualUpdate)
#else
TEST_P(GpuRadarSensorTest, ManualUpdate)
#endif
{
...
/////////////////////////////////////////////////
TEST_P(GpuRadarSensorTest, Topic)
{
  Topic(GetParam());
}

INSTANTIATE_TEST_CASE_P(GpuRadarSensor, GpuRadarSensorTest,
