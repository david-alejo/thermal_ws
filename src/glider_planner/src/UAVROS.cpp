...
UAV* UAVROS::clone() const
{
  ...
  uav_real.waypoint_client = n.serviceClient<mavros_msgs::WaypointPush>(flight_plan_filename);
  uav_real.cur_wp_client = n.serviceClient<mavros_msgs::WaypointSetCurrent &>(cur_wp_service);
  uav_real.param_client = n.serviceClient<mavros_msgs::ParamSet &>(param_service);
  ...
  boost::function<void (const mavros_msgs::BatteryStatus &)> fun_bat = 
			      boost::bind(&UAVROS::batteryCallback, dynamic_cast<UAVROS*>(new_uav), _1);
  boost::function<void (const mavros_msgs::VFR_HUD &)> fun_vfr = 
			      boost::bind(&UAVROS::VFRCallback, dynamic_cast<UAVROS*>(new_uav), _1);
  uav_real.battery_subscriber = n.subscribe<mavros_msgs::BatteryStatus &>(battery_topic, 1, fun_bat);
  uav_real.VFR_subscriber =n.subscribe<mavros_msgs::VFR_HUD &>(VFR_topic,1,fun_vfr);
...
void UAVROS::setFlightPlan(const simulator::FlightPlan& new_plan)
{
  ...
  if (new_plan.isDifferent(old_plan, 1, min_dist) ) {
    ...
    if (actualize) {
      ...
      mavros_msgs::WaypointSetCurrent srv_current;
...
bool UAVROS::setThrMaxRetry(double thr)
{
  ...
  if (throttle_percentage != thr) {
    ...
    mavros_msgs::ParamSet srv_param;
...
bool UAVROS::init(ParseBlock& block)
{
  ...
  try {
	...
	min_dist = 50.0;
      ...
      // ROS connections
      waypoint_client = n.serviceClient<mavros_msgs::WaypointPush>(flight_plan_filename);
      cur_wp_client = n.serviceClient<mavros_msgs::WaypointSetCurrent>(cur_wp_service);
      param_client = n.serviceClient<mavros_msgs::ParamSet &>(param_service);
      boost::function<void (const sensor_msgs::NavSatFix &)> fun = 
      ...
      battery_subscriber = n.subscribe<mavros_msgs::BatteryStatus &>(battery_topic, 1, fun_bat);
      VFR_subscriber =n.subscribe<mavros_msgs::VFR_HUD &>(VFR_topic,1,fun_vfr);
...
void UAVROS::batteryCallback(const mavros_msgs::BatteryStatus& battery)
{
  this->battery = battery;
}
void UAVROS::VFRCallback(const mavros_msgs::VFR_HUD& state)
{
  this->status=state;
}
...
mavros_msgs::WaypointList UAVROS::toMavRos(const UAVFlightPlan::UAVFlightPlan &fp, bool &in_updraft) const
{
  mavros_msgs::WaypointList ret;
  in_updraft = false;
  UAVFlightPlan::UAVFlightPlan::const_iterator it = fp.begin();
  mavros_msgs::Waypoint w_mav;
  for (unsigned int way_id = 0; it != fp.end(); way_id++, it++) {
    ...
    if (!w.getReachAltitude()) {
      ...
      way_id++;
      // Then the jump
      mavros_msgs::Waypoint w_mav_jump = jumpCommand(way_id + 2, 10);
...
//       w_mav.param1 = it->getClimbRate();
  ...
  if (fp.getLoop() && fp.size() > 0) {
    mavros_msgs::Waypoint w_mav_jump = jumpCommand(0, 10);
    ret.waypoints.push_back(w_mav_jump);
    
    mavros_msgs::Waypoint w_mav_finish = w_mav; // Include a dummy waypoint
...
mavros_msgs::Waypoint UAVROS::jumpCommand(uint target_id, uint n_times) const
{
